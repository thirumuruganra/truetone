    ...
    <form className="space-y-8" onSubmit={handleSubmit}>
      <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <label className="space-y-3">
          <span className="text-sm font-semibold uppercase tracking-[0.18em] text-[color:var(--color-muted)]">
            Identity
          </span>
          <textarea
            value={form.identity}
            ...
            rows={4}
            className="w-full rounded-[1.5rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] px-5 py-4 text-base leading-7 text-[color:var(--color-foreground)] outline-none transition focus:border-[color:var(--color-accent)]"
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <label className="space-y-3 lg:col-span-1">
          ...
          <input
            ...
            className="w-full rounded-[999px] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] px-5 py-4 text-base text-[color:var(--color-foreground)] outline-none transition focus:border-[color:var(--color-accent)]"
      ...
      <section className="space-y-4 rounded-[2rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-5 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[color:var(--color-muted)]">
              Example posts
            </p>
            <h3 className="font-display text-3xl leading-none text-[color:var(--color-foreground)]">
              Add 2 to 5 posts that sound most like you.
            </h3>
          </div>
          <p className="text-sm text-[color:var(--color-muted)]">{completedExamples} of 5 added</p>
        </div>

        <div className="grid gap-4">
          {form.examplePosts.map((post, index) => (
            <label key={index} className="space-y-2">
              <span className="text-sm font-medium text-[color:var(--color-muted)]">Post {index + 1}</span>
              <textarea
                value={post}
                onChange={(event) => updateExamplePost(index, event.target.value)}
                rows={5}
                className="w-full rounded-[1.5rem] border border-[color:var(--color-border)] bg-[color:var(--color-page)] px-5 py-4 text-base leading-7 text-[color:var(--color-foreground)] outline-none transition focus:border-[color:var(--color-accent)]"
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <p aria-live="polite" className="text-sm text-[color:var(--color-danger)]">{feedback}</p>
        <button
          type="submit"
          disabled={isPending}
          className="min-h-11 w-full rounded-full bg-[color:var(--color-accent)] px-6 py-3 text-sm font-semibold text-[color:var(--color-accent-foreground)] transition hover:opacity-92 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
        >
